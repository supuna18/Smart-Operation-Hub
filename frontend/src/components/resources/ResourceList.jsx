        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden">
            <div className="p-6 md:p-8 bg-[#262626] border-b border-white/5 flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <p className="text-sm text-slate-400 font-medium">Manage campus resources from a single, high-performance interface.</p>
                </div>
                
                <div className="flex flex-col md:flex-row gap-3 relative z-10 w-full md:w-auto">
                    {/* FIXED NAVIGATION PATH: Matches App.jsx (/my-bookings) */}
                    {!admin && (
                        <button 
                        ...
                        >
                            <FiBox size={18} /> My Reservation Details
                        </button>
                    )}

                    {admin && (
                        <button onClick={onAdd} className="flex items-center justify-center gap-2 bg-[#FACC15] hover:bg-yellow-400 text-slate-900 px-7 py-3 rounded-xl transition-all font-bold active:scale-95 shadow-lg">
            ...
            <div className="p-6 md:p-8">
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {resources.map((resource) => (
                            <div key={resource.id} className="group bg-white border border-slate-200 rounded-xl overflow-hidden hover:border-yellow-400 transition-all flex flex-col shadow-sm">
                                <div className={`h-1.5 w-full ${resource.status === 'ACTIVE' ? 'bg-emerald-500' : 'bg-yellow-500'}`} />
                                <div className="h-44 w-full relative overflow-hidden bg-slate-100">
                                    <img src={resource.imageUrl || getTypeDefaultImage(resource.type)} alt={resource.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                                </div>
                                ...
                                <div className="px-5 py-4 flex gap-2 border-t border-slate-50">
                                    ...
                                    {!admin && (
                                        <button onClick={() => handleBook(resource)} className="flex-1 flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 text-white py-2 rounded-lg font-bold text-xs shadow-sm" disabled={resource.status !== 'ACTIVE'}>
                                            Book Now
                                        </button>
                                    )}
                                </div>
                                ...
                                {!admin && getBookingForResource(resource.id) && (
                                    <div className="px-5 pb-4">
                                        <div className={`w-full flex flex-col items-center justify-center py-2 rounded-lg border bg-opacity-5 font-bold ${
                                            ...
                                            <span className="text-[10px] uppercase opacity-60">Latest status</span>
                ...
                {calendarResource && <ResourceCalendarModal resource={calendarResource} onClose={() => setCalendarResource(null)} />}
