...
const ResourceList = ({ onEdit, onAdd }) => {
    ...
    // FIXED: No more orphaned code or extra braces
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden">
            <div className="p-6 md:p-8 bg-[#262626] border-b border-white/5 flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <p className="text-sm text-slate-400 font-medium">Manage and optimize campus resources from a single interface.</p>
                </div>
                {admin && (
                    <button onClick={onAdd} className="relative z-10 w-full md:w-auto flex items-center justify-center gap-2 bg-[#FACC15] hover:bg-yellow-400 text-slate-900 px-7 py-3 rounded-xl transition-all font-bold active:scale-95">
                        <FiPlus size={18} className="stroke-[3px]" /> New Resource
                    </button>
                )}
            </div>

            {/* Filters section stays same */}
            <div className="p-6 md:p-8 bg-slate-50/50 border-b border-slate-100 backdrop-blur-sm">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
                    <div className="relative">
                        <FiSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                        <input type="text" placeholder="Search..." className="w-full pl-11 pr-4 py-2.5 bg-white border border-slate-300 rounded-lg text-sm" value={search} onChange={(e) => setSearch(e.target.value)} />
                    </div>
                    {/* ... other filters ... */}
                </div>
            </div>
            ...
            <div className="p-6 md:p-8">
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {resources.map((resource) => (
                            <div key={resource.id} className="group bg-white border border-slate-200 rounded-xl overflow-hidden hover:border-yellow-400 transition-all flex flex-col">
                                <div className={`h-1.5 w-full ${resource.status === 'ACTIVE' ? 'bg-emerald-500' : 'bg-yellow-500'}`} />
                                <div className="h-44 w-full relative overflow-hidden bg-slate-100">
                                    <img src={resource.imageUrl || getTypeDefaultImage(resource.type)} alt={resource.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                                </div>

                                <div className="p-5 flex-1">
                                    <span className="inline-block px-2.5 py-1 bg-slate-100 text-slate-600 text-[11px] font-bold uppercase rounded-md mb-2">{resource.type}</span>
                                    <h3 className="text-lg font-bold text-slate-900 leading-tight">{resource.name}</h3>
                                    <div className="space-y-2 mt-4 text-sm text-slate-600">
                                        <div className="flex items-center gap-2"><FiUsers size={14} /> <span>{resource.capacity} Seats</span></div>
                                        <div className="flex items-center gap-2"><FiMapPin size={14} /> <span>{resource.location}</span></div>
                                    </div>
                                </div>

                                <div className="px-5 py-4 flex gap-2 border-t border-slate-50">
                                    <button onClick={() => setCalendarResource(resource)} className="flex-1 flex items-center justify-center gap-2 bg-slate-50 hover:bg-slate-100 text-slate-600 py-2 rounded-lg font-bold text-xs">
                                        <FiCalendar size={14} /> Schedule
                                    </button>
                                    {!admin && !getBookingForResource(resource.id) && (
                                        <button onClick={() => handleBook(resource)} className="flex-1 flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 text-white py-2 rounded-lg font-bold text-xs" disabled={resource.status !== 'ACTIVE'}>
                                            Book Now
                                        </button>
                                    )}
                                </div>
                                ...
                                {!admin && getBookingForResource(resource.id) && (
                                    <div className="px-5 pb-4">
                                        <div className="w-full flex flex-col items-center justify-center py-2 rounded-lg border bg-yellow-50 text-yellow-700 font-bold">
                                            <span className="text-[10px] uppercase opacity-60">Your Booking Status</span>
                ...
                {calendarResource && (
                    <ResourceCalendarModal resource={calendarResource} onClose={() => setCalendarResource(null)} />
                )}
